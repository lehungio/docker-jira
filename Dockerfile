...
RUN mkdir -p ${JIRA_INSTALL_DIR} \
  ...
  && sed -i -e 's/port="8086"/port="8086" secure="${catalinaConnectorSecure}" scheme="${catalinaConnectorScheme}" proxyName="${catalinaConnectorProxyName}" proxyPort="${catalinaConnectorProxyPort}"/' ${JIRA_INSTALL_DIR}/conf/server.xml \
